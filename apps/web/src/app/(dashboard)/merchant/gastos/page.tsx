...
import { Receipt, Plus, X, Pencil, Trash2, DollarSign, Calendar, Landmark, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function MerchantGastosPage() {
  ...
  useEffect(() => {
    if (store?.plan === 'PRO') {
      loadExpensesAndScheduled();
    }
  }, [selectedMonth, selectedYear, store?.plan]);
  ...
  const loadStore = async () => {
    try {
      ...
      toast.error('Error cargando el comercio');
      setLoading(false);
    }
  };

  const loadExpensesAndScheduled = async () => {
    try {
  ...
  return (
    <div className="space-y-6 font-sans pb-10">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-gray-150 dark:border-gray-800 pb-5 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5 font-medium">Registra y controla las salidas de caja, egresos operativos y deudas</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border border-red-100 dark:border-red-900/30 bg-red-50/10 p-5 shadow-xs">
          <p className="text-xs text-red-500/80 font-bold uppercase tracking-wider">Saldo Restante por Pagar</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-150/45 dark:border-gray-700/60 rounded-3xl shadow-sm p-6 lg:col-span-7 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 dark:border-gray-750 pb-4 gap-2">
            <h3 className="font-extrabold text-base">Historial de Egresos</h3>
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-700 p-1 rounded-xl text-xs font-bold self-start">
              <button onClick={() => setFilter('ALL')} className={`px-3 py-1.5 rounded-lg transition ${filter === 'ALL' ? 'bg-white dark:bg-gray-850 text-gray-900 dark:text-white shadow-xs' : 'text-gray-500'}`}>Todos</button>
              <button onClick={() => setFilter('DAILY')} className={`px-3 py-1.5 rounded-lg transition ${filter === 'DAILY' ? 'bg-white dark:bg-gray-850 text-gray-900 dark:text-white shadow-xs' : 'text-gray-500'}`}>Diarios</button>
              <button onClick={() => setFilter('MONTHLY')} className={`px-3 py-1.5 rounded-lg transition ${filter === 'MONTHLY' ? 'bg-white dark:bg-gray-850 text-gray-900 dark:text-white shadow-xs' : 'text-gray-500'}`}>Mensuales</button>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-750 font-semibold text-gray-700 dark:text-gray-300">
                    {filtered.map(e => (
                      <tr key={e.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-750/30 transition">
                        ...
                        <td className="py-3.5">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] ${e.type === 'DAILY' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400'}`}>
                            {e.type === 'DAILY' ? 'Diario' : 'Mensual'}
                          </span>
                        </td>
                        <td className="py-3.5 font-black text-gray-900 dark:text-white">{formatCurrency(e.amount)}</td>
                        <td className="py-3.5 text-right">
                          <div className="flex justify-end gap-1.5">
                            ...
                            <button onClick={() => handleDelete(e.id)} className="p-1.5 hover:bg-red-55 dark:hover:bg-red-950/30 text-red-500 rounded-lg transition" title="Eliminar"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="md:hidden space-y-3">
                {filtered.map(e => (
                  <div key={e.id} className="p-4 bg-gray-50 dark:bg-gray-750/30 border border-gray-100 dark:border-gray-700 rounded-2xl space-y-2">
                    ...
                    {e.description && <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">{e.description}</p>}
                    <div className="flex justify-between items-center pt-1.5 border-t border-gray-100 dark:border-gray-700/50">
                      <span className="font-black text-gray-900 dark:text-white text-base">{formatCurrency(e.amount)}</span>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-150/45 dark:border-gray-700/60 rounded-3xl shadow-sm p-6 lg:col-span-5 space-y-5">
          <div className="border-b border-gray-100 dark:border-gray-750 pb-4">
            ...
            <p className="text-xs text-gray-400 font-medium mt-0.5">Gastos pendientes y cuentas por pagar a futuro.</p>
          </div>

          {scheduled.length === 0 ? (
            <p className="text-center py-12 text-gray-400 text-sm font-medium">No hay deudas programadas registradas en este mes.</p>
          ) : (
                    ...
                    <div className="pl-2.5 space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {item.description && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 font-medium">{item.description}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {!isPaid && (
                            <button
                              ...
                              className="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-xs font-extrabold transition shadow-xs"
                            >
                              Abonar
                            </button>
                          )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-150 dark:border-gray-700/80 p-6 w-full max-w-md shadow-xl space-y-4 animate-scale-up">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder={form.type === 'SCHEDULED' ? 'Ej: Proveedor de Carnes, Arriendo' : 'Ej: Insumos, Servicios, Alquiler'}
                  value={form.category}
              ...
              <div>
                <label className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1 block">Descripción</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1 block">Monto ($) *</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1 block">Tipo de Registro</label>
                <select
                  value={form.type}
      ...
      {showPayModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-155 dark:border-gray-700/80 p-6 w-full max-w-sm shadow-xl space-y-4 animate-scale-up">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-gray-550 font-bold uppercase tracking-wider mb-1 block">Monto del Abono ($)</label>
                <input
                  type="number"

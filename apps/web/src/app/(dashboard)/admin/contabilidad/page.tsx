...
type Tab = 'daily' | 'weekly' | 'monthly' | 'cashclose';
  ...
  const loadData = async () => {
    ...
    try {
      if (tab === 'daily') {
        ...
        const data = await api.get<WeeklyReport>(`/accounting/weekly?date=${selectedDate}`);
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {[
          ...
          { id: 'weekly' as Tab, label: 'Semanal' },
      ...
      {tab === 'weekly' && (
        <div>
          <div className="flex flex-col md:flex-row md:items-center gap-3 mb-6">
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm"
              />
            </div>
            {weeklyReport && (
              <span className="text-sm font-semibold text-gray-500 bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-lg">
                Semana: {formatWeeklyRange(weeklyReport.startDate, weeklyReport.endDate)}
              </span>
            )}
          </div>
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <MetricCard label="Ventas de la Semana" value={formatCurrency(weeklyReport.totalSales)} icon={DollarSign} color="text-green-500" />
                <MetricCard label="Utilidad Neta" value={formatCurrency(weeklyReport.netProfit)} icon={weeklyReport.netProfit >= 0 ? TrendingUp : TrendingDown} color={weeklyReport.netProfit >= 0 ? 'text-green-500' : 'text-red-500'} />
                <MetricCard label="Margen" value={`${weeklyReport.margin}%`} icon={TrendingUp} color="text-blue-500" />
                <MetricCard label="Pedidos" value={String(weeklyReport.orderCount)} icon={Calculator} color="text-purple-500" />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
                  <h3 className="font-bold mb-3">Ingresos y Créditos</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between font-semibold"><span>Ventas ({weeklyReport.orderCount} pedidos)</span><span className="text-green-500">{formatCurrency(weeklyReport.totalSales)}</span></div>
                    {weeklyReport.salesByMethod.map(s => (
                      <div key={s.method} className="flex justify-between text-gray-500 pl-4"><span>{s.method}</span><span>{formatCurrency(s.amount)}</span></div>
                    ))}
                    <div className="border-t my-2 dark:border-gray-700"></div>
                    <div className="flex justify-between font-medium"><span>Abonos de Fiados (Recaudado)</span><span className="text-green-600 font-bold">{formatCurrency(weeklyReport.totalAbonos)}</span></div>
                    <div className="flex justify-between font-medium text-gray-500"><span>Fiado en la semana (Por Cobrar)</span><span className="text-purple-650 font-bold">{formatCurrency(weeklyReport.totalFiado)}</span></div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
                  <h3 className="font-bold mb-3">Egresos</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span>Costo proveedores</span><span className="text-red-500">{formatCurrency(weeklyReport.supplierCosts)}</span></div>
                    <div className="flex justify-between"><span>Mermas</span><span className="text-red-500">{formatCurrency(weeklyReport.wasteCost)}</span></div>
                    <div className="flex justify-between"><span>Gastos semanales</span><span className="text-red-500">{formatCurrency(weeklyReport.dailyExpenses)}</span></div>
                    <div className="flex justify-between"><span>Fijos prorrateados (7 días)</span><span className="text-red-500">{formatCurrency(weeklyReport.weeklyExpensesProrated)}</span></div>
                    <div className="border-t pt-2 flex justify-between font-bold"><span>Total egresos</span><span className="text-red-500">{formatCurrency(weeklyReport.supplierCosts + weeklyReport.wasteCost + weeklyReport.dailyExpenses + weeklyReport.weeklyExpensesProrated)}</span></div>
                  </div>
                </div>
              </div>
            </>
...
function formatWeeklyRange(startStr: string, endStr: string) {
  const start = new Date(startStr);
  const end = new Date(endStr);
  if (isNaN(start.getTime()) || isNaN(end.getTime())) return '';
  const options: Intl.DateTimeFormatOptions = { weekday: 'short', day: 'numeric', month: 'short', timeZone: 'America/Bogota' };

        ...
        {tables.map((t) => (
          <div key={t.id} className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              ...
              <a href={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/tables/${t.id}/qr?frontendUrl=${typeof window !== 'undefined' ? encodeURIComponent(window.location.origin) : ''}`} download className="flex-1 py-2 text-xs bg-green-50 dark:bg-green-900/20 text-green-600 rounded-lg font-medium flex items-center justify-center gap-1"><Download size={12} /> PNG</a>
              <a href={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/tables/${t.id}/qr?format=pdf&frontendUrl=${typeof window !== 'undefined' ? encodeURIComponent(window.location.origin) : ''}`} download className="flex-1 py-2 text-xs bg-purple-50 dark:bg-purple-900/20 text-purple-600 rounded-lg font-medium flex items-center justify-center gap-1"><Download size={12} /> PDF</a>
